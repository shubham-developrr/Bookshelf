...
const EnhancedReaderPage: React.FC<EnhancedReaderPageProps> = ({ 
    ...
    const [activeTab, setActiveTab] = useState('edit'); // Default to edit tab (formerly read)
    ...
    const handleContentAndImagesUpdate = (subtopicId: string, content: string, images: string[], imageLabels?: string[]) => {
        const savedBooks = JSON.parse(localStorage.getItem('createdBooks') || '[]');
        const customBook = savedBooks.find((book: any) => book.name === currentBook);
        if (!customBook) return;
        ...
        const updatedSubtopics = customSubtopics.map(sub => 
            sub.id === subtopicId 
                ? { ...sub, content, images, imageLabels: imageLabels || [] }
                : sub
        );
...
Make the explanation educational and easy to understand.`;
    ...
    const handleEditVideo = (subtopic: string) => {
        // Open the settings modal to add/edit videos
        openVideoModal(subtopic, null, 'edit'); // Always open in edit mode for adding/managing videos
    };
    ...
    // Clean read content - no editing features, for final reading experience
    const renderCleanReadContent = () => {
        return (
            <div>
                {/* Subtopics - Clean Reading Experience */}
                            ...
                            {allSubtopics.map((subtopic, index) => {
                                ...
                                return (
                                    <div key={index} className="theme-transition">
                                        ...
                                        <div className="theme-surface rounded-lg overflow-hidden theme-transition">
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    {isCustomChapter && subtopicData?.videoLinks && subtopicData.videoLinks.length > 0 && (
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                        ...
                                        {/* Content Section - Clean Reading */}
                                        {expandedSubtopics.has(subtopic) && (
                                            <div className="mt-6 mb-8 px-0">
                                                {/* Clean Content Display for Custom Subtopics */}
                                                {isCustomChapter && subtopicData && (
                                                    <div className="prose prose-sm sm:prose-base lg:prose-lg max-w-none theme-text">
                                                        <div className="space-y-4">
                                                            {subtopicData.content.split('\n\n').map((textBlock, textIndex) => (
                                                                <div key={`text-${textIndex}`}>
                                                                    {textBlock && (
                                                                        <div className="whitespace-pre-wrap">{textBlock}</div>
                                                                    )}
                                                                    
                                                                    {/* Render images inline with labels */}
                                                                    {subtopicData.images.slice(textIndex, textIndex + 1).map((img, imgRelativeIndex) => {
                                                                        const imgIndex = textIndex + imgRelativeIndex;
                                                                        const imageLabel = subtopicData.imageLabels?.[imgIndex];
                                                                        return (
                                                                            <div key={`image-${imgIndex}`} className="my-4 text-center">
                                                                                <img
                                                                                    src={img}
                                                                                    alt={imageLabel || `Content image ${imgIndex + 1}`}
                                                                                    className="max-w-full h-auto max-h-96 object-contain rounded-lg shadow-md mx-auto"
                                                                                />
                                                                                {imageLabel && (
                                                                                    <p className="mt-2 text-sm theme-text-secondary italic">
                                                                                        {imageLabel}
                                                                                    </p>
                                                                                )}
                                                                            </div>
                                                            ...
                                                            {/* Remaining images */}
                                                            {subtopicData.images.slice(subtopicData.content.split('\n\n').length).map((img, imgIndex) => {
                                                                const actualIndex = subtopicData.content.split('\n\n').length + imgIndex;
                                                                const imageLabel = subtopicData.imageLabels?.[actualIndex];
                                                                return (
                                                                    <div key={`image-remaining-${actualIndex}`} className="my-4 text-center">
                                                                        <img
                                                                            src={img}
                                                                            alt={imageLabel || `Content image ${actualIndex + 1}`}
                                                                            className="max-w-full h-auto max-h-96 object-contain rounded-lg shadow-md mx-auto"
                                                                        />
                                                                        {imageLabel && (
                                                                            <p className="mt-2 text-sm theme-text-secondary italic">
                                                ...
                                                {/* Static Content for Legacy Subtopics with Highlighting Support */}
                                                {!isCustomChapter && (
                                                    <div 
                                                    ...
                                                    >
                                                        <KindleStyleTextViewer
                                                            content={subtopicContent[subtopic] || `Content for "${subtopic}" will be available soon. This is a comprehensive educational platform designed to provide in-depth coverage of all topics.`}
                                                            highlights={highlights}
                                                            onHighlight={(text: string, color: string) => addHighlight({ text, color, chapterId: currentChapter })}
                                                            onRemoveHighlight={removeHighlight}
                                                            onExplainWithAI={(text: string) => openAIGuru(`Please explain this text: "${text}"`)}
                            ...
                            {allSubtopics.map((subtopic, index) => {
                                ...
                                return (
                                    <div key={index} className="theme-transition">
                                                    ...
                                                    {isCustomChapter && subtopicData && (
                                                        <>
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    // Make the Edit button work like Edit Content
                                                                    // This will expand the subtopic if collapsed and enable editing
                                                                    if (!expandedSubtopics.has(subtopic)) {
                                                                        setExpandedSubtopics(prev => new Set([...prev, subtopic]));
                                                                    }
                                                                    // Force the inline editor to open in edit mode
                                                                    setTimeout(() => {
                                                                        const editButton = document.querySelector(`[data-subtopic-id="${subtopicData.id}"] .edit-content-button`) as HTMLButtonElement;
                                                                        if (editButton) {
                                                                            editButton.click();
                                                                        }
                                                                    }, 100);
                                                            ...
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    // Create a temporary file input for this specific subtopic
                                                                    ...
                                                                    input.onchange = (event) => {
                                                                        ...
                                                                        if (file) {
                                                                            handleImageUpload(subtopicData.id, file);
                                                                        }
                                                                    };
                                        ...
                                        {/* Content Section */}
                                        {expandedSubtopics.has(subtopic) && (
                                            <div className="mt-6 mb-8 px-0" data-subtopic-id={subtopicData?.id}>
                                                ...
                                                {isCustomChapter && subtopicData && (
                                                    <InlineContentEditor
                                                        ...
                                                        imageLabels={subtopicData.imageLabels}
                                                        onContentUpdate={(content, images, imageLabels) => 
                                                            handleContentAndImagesUpdate(subtopicData.id, content, images, imageLabels)
                                                        }
